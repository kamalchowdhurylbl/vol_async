...
status =
    H5Dwrite_async(dataset, H5T_NATIVE_INT, memspace, dataspace, data_transfer_propertylist, data, es_id);
    ...
    DL_FOREACH(task_list_iter->task_list, task_iter)
    {
        if (task_iter->func == async_dataset_write_fn) {
            ...
            if (task_iter->parent_obj == parent_obj) {
                ...
                fprintf(stderr, "%lld   %lld  %lld\n", task_iter->async_obj->under_object,
                        iter_args->mem_space_id, iter_args->file_space_id);
