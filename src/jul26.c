                 ...
                 hsize_t *count)
{
    ...
    if (current_task_type == H5S_SEL_HYPERSLABS) {
        ...
        if (H5Sis_regular_hyperslab(current_task_space_id)) {
            ...
            /*  free(current_task_start_out);
             free(current_task_stride_out);
             free(current_task_count_out);
             free(current_task_block_out);

             free(iterator_task_start_out);
             free(iterator_task_stride_out);
             free(iterator_task_count_out);
             free(iterator_task_block_out); */
        }
    }
    return 0;
}
