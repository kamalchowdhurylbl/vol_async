...
main(void)
{
    ...
    file = H5Fcreate(FILE, H5F_ACC_TRUNC, H5P_DEFAULT,
                     H5P_DEFAULT); // H5F_ACC_TRUNC	If the file already exists, the file is
    // opened with read-write access, and new data will overwrite any existing data. If the file does not
     ...
     */
    mid1 = H5Screate_simple(MSPACE1_RANK, dim1, NULL); // MSPACE1_RANK=1  dim1=50 H5Screate_simple=Creates a
                                                       // new simple dataspace and opens it for access.
    ...
    block[0]  = 1;
    ret =
        H5Sselect_hyperslab(mid1, H5S_SELECT_SET, start, stride, count,
                            block); // H5S_SELECT_SET	Replaces the existing selection with the parameters from
    ...
    ret = H5Dwrite(dataset, H5T_NATIVE_INT, mid1, fid, H5P_DEFAULT,
                   vector); // H5Dwrite=Writes raw data from a buffer to a dataset.
    /*
     * Read data back to the buffer matrix.
     */
    ret = H5Dread(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                  matrix); // H5Dread=Reads raw data from a dataset into a buffer.
     ...
     */
    mid2 = H5Screate_simple(MSPACE2_RANK, dim2, NULL); // MSPACE2_RANK=1  ,dim2= 4  H5Screate_simple=Creates a
                                                       // new simple dataspace and opens it for access.
    ...
    ret =
        H5Sselect_elements(fid, H5S_SELECT_SET, NPOINTS,
