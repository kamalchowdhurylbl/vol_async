...
main(void)
{
     ...
     */
    status = H5Dwrite_async(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data,
                            es_id); // H5T_NATIVE_INT = C-style int

    status = H5Dread_async(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data_out1, es_id);
     ...
     */
    file = H5Fopen_async(FILE, H5F_ACC_RDONLY, H5P_DEFAULT,
                         es_id); // H5F_ACC_RDONLY= An existing file is opened with read-only access.
     ...
     * memory and display.
     */
    status = H5Dread_async(dataset, H5T_NATIVE_INT, memspace, dataspace, H5P_DEFAULT, data_out, es_id);
