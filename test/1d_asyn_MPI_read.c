...
main(int argc, char **argv)
{
    ...
    if (mpi_rank == 0) {
        ...
        dataset = H5Dcreate_async(file1d, DATASETNAME, H5T_STD_I32BE, dataspace, H5P_DEFAULT, H5P_DEFAULT,
                                  H5P_DEFAULT, es_id); // H5T_STD_I32BE = 32-bit big-endian signed integers

        status = H5Dwrite_async(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data1d,
                                es_id); // H5T_NATIVE_INT = C-style int

        status = H5Dread_async(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data_out1d, es_id);
   ...
   ************************************************************/
     ...
     */
    file1d = H5Fopen_async(FILE1D, H5F_ACC_RDONLY, mpio_plist_id,
