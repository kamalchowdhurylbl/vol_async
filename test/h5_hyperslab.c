...
main(void)
{
     ...
     */
    dataset = H5Dcreate(file, DATASETNAME, H5T_STD_I32BE, dataspace, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT); // H5T_STD_I32BE = 32-bit big-endian signed integers

    /*
     * Write the data to the dataset using default transfer properties.
     */
    status = H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                      data); // H5T_NATIVE_INT = C-style int

    status = H5Dread(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data_out1);

    printf("First Data:\n ");

    for (j = 0; j < X; j++) {
        for (i = 0; i < Y; i++)
            printf("%d ", data_out1[j][i]);
        printf("\n ");
    }
    printf("\n");
    /*
     ...
     */
    file = H5Fopen(FILE, H5F_ACC_RDONLY,
                   H5P_DEFAULT); // H5F_ACC_RDONLY= An existing file is opened with read-only access.
    ...
    */
    ...
    // H5S_SELECT_SET=Replace the existing selection with the parameters from this call. Overlapping blocks
    // are not supported with this operator. H5Sselect_hyperslab selects a hyperslab region to add to the
    // current selected region for a specified dataspace. offset value from row 1, col 2
...
xfer_plist_id, void * buf ) Purpose: Reads raw data from a dataset into a buffer. Description: H5Dread reads a
(partial) dataset, specified by its identifier dataset_id, from the file into an application memory buffer
buf. Data transfer properties are defined by the argument xfer_plist_id. The memory datatype of the (partial)
dataset is identified by the identifier mem_type_id. The part of the dataset to read is defined by
mem_space_id and file_space_id. file_space_id is used to specify only the selection within the file dataset's
dataspace. Any dataspace specified in file_space_id is ignored by the library and the dataset's dataspace is
always used. file_space_id can be the constant H5S_ALL. which indicates that the entire file dataspace, as
